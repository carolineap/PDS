...
end = datetime.datetime.strptime("2018-10-08", '%Y-%m-%d')
	...
	while (not request):
		try:
			...
			if (table):
				...
				file.write("Data; Mercadoria; Vencimento; Preco Anterior; Preco Atual\n")
				...
				for tr in table_rows:
					...
					if (mercadoria[:3] == 'BGI' or mercadoria[:3] == 'ACF' or mercadoria[:3] == 'ICF' or mercadoria[:3] == 'CCM'):
						file.write(str(start.day) + str(start.month)  + str(start.year) + '; ' + mercadoria[:3] + '; ' + vencimento[:3] + '; ' + precoAnterior + '; ' + precoAtual + '\n')
